...
public class InfoCommand implements CommandExecutor,TabCompleter, Listener {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		HoneyfrostWorld honeyfrostWorld = HoneyfrostWorld.getWorld(world.getName());
		Project project = (honeyfrostWorld != null ? honeyfrostWorld.getProject() : null);

		sender.sendMessage("§8§m-----§r §3§l" + Message.WORD_INFORMATION.get(sender) + " §8§m-----");
		sender.sendMessage("§8§l» §7" + Message.WORD_WORLD.get(sender) + ": §b" + (honeyfrostWorld != null ? honeyfrostWorld.getDisplayname() + " §8" + honeyfrostWorld.getFileWorldName(): world.getName()));
